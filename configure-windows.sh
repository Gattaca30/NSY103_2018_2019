...
do
	curl -s test.yml -o ~/.docker/docker-compose-${i}.yml && echo "docker-compose-$i.yml updated" || echo failed to get docker-compose-$i.yml
        sed -i "test"  ~/.docker/docker-compose-${i}.yml
done
